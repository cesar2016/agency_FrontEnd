...
export default function UsersPage() {
  ...
  const loadCaja = useCallback(async (userId, date) => {
    ...
    try {
      ...
      setCajaData(null);
      flash(e?.response?.data?.message || 'Error al cargar caja');
    } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {cajaUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={closeCaja}>
          <div
          ...
          >
            <div className="flex items-start justify-between mb-4 gap-3">
              <div>
                ...
                <p className="text-sm text-indigo-300 mt-0.5">{cajaUser.name}</p>
                {cajaUser.username && (
                  <p className="text-xs text-gray-500 font-mono">@{cajaUser.username}</p>
                )}
              </div>
              <button onClick={closeCaja} className="text-gray-400 hover:text-white transition">
                <FiX size={18} />
              </button>
            </div>
            ...
            <div className="flex gap-2 items-end mb-4">
              ...
              <button
                type="button"
                onClick={() => loadCaja(cajaUser.id, cajaDate)}
